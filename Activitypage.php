...
$sql = 'SELECT * FROM activitiesattendance order by AID ASC';
$result = mysqli_query($conn ,$sql);
while ($row = mysqli_fetch_assoc($result)) {
  if($_SESSION['University_ID'] == $row['ATID']){
    echo  "<form method='POST'>
    <input type='hidden' name='SID' value='".$row['SID']."' />
  <input type='hidden' name='AID' value='".$row['AID']."' />
    <input type='submit' name='updataSkilles' value='".$row['AID']."''> ".$row['SID']." <br></form>
     ";
  }
}
$count = mysqli_num_rows($result);
// echo $count;
function updataSkilles($conn,$SID,$AID) {
  if (isset($_POST['updataSkilles'])) {
    
    $sql = "SELECT skill_acquired FROM activities WHERE id = '$AID'";
    $result = mysqli_query($conn,$sql);
    $row = mysqli_fetch_array($result);
    print_r($row);
    //$sql= "SELECT SID FROM activitiesattendance WHERE AID = '$AID'";
   // $AID=NULL;
    //$result = mysqli_query($conn,$sql);
    //$SID = mysqli_fetch_array($result);
echo $SID;
    $sql = "UPDATE user_details SET skill='".$row[0]."' WHERE UniversityID = '$SID'";
    $result = mysqli_query($conn,$sql);
    //$row = mysqli_fetch_array($result);
    $SID=NULL;
  }
}
if(array_key_exists('updataSkilles',$_POST)){
  $btnValue = $_POST['SID'];
  $SID = $_POST['SID'];
